    ...
    public class WxPayApi
    {
        ...
        */
        ...
        public static WxPayData AppOrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            if (!inputObj.IsSet("out_trade_no") && !inputObj.IsSet("transaction_id"))
            {
                throw new WxPayException("App订单查询接口中，out_trade_no、transaction_id至少填一个！");
            }

            inputObj.SetValue("appid", WxAppPayConfig.APPID);//公众账号ID
            inputObj.SetValue("mch_id", WxAppPayConfig.MCHID);//商户号
            inputObj.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            ...
            Log.Debug("WxPayApi", "AppOrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            Log.Debug("WxPayApi", "AppOrderQuery response : " + response);
        ...
        */
        ...
        public static WxPayData UnifiedAppOrder(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no"))
            ...
            if (!inputObj.IsSet("notify_url"))
            {
                inputObj.SetValue("notify_url", WxAppPayConfig.NOTIFY_URL);//异步通知url
            }

            if ( !inputObj.IsSet( "spbill_create_ip" ) )
            {
                inputObj.SetValue( "spbill_create_ip", WxAppPayConfig.IP );//终端ip
            }
            ...
            ////签名
            inputObj.SetValue("sign", inputObj.MakeAppSign());
            //if (!inputObj.IsSet("sign"))
            //{
            //    inputObj.SetValue("sign", inputObj.MakeSign());//签名
            //}
            ...
            Log.Debug("WxPayApi", "UnfiedAppOrder request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            Log.Debug("WxPayApi", "UnfiedAppOrder response : " + response);
        ...
        public static WxPayData UnifiedOrderSandBox(WxPayData inputObj, string key, int timeOut = 6)
        {
            ...
            //签名
            inputObj.SetValue("sign", inputObj.MakeSandboxSign(key));
            string xml = inputObj.ToXml();
        ...
        public static WxPayData GetSandBoxSignKey(WxPayData inputObj, int timeOut = 6)
        {
       ...
       * @return 成功时返回接口调用结果，其他抛异常
       */
        public static WxPayData AppRefund(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/secapi/pay/refund";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no") && !inputObj.IsSet("transaction_id"))
            ...
            Log.Debug("WxPayApi", "Refund request : " + xml);
            string response = AppHttpService.Post(xml, url, true, timeOut);//调用HTTP通信接口提交数据到API
            Log.Debug("WxPayApi", "Refund response : " + response);
	    ...
	    * @return 成功时返回，其他抛异常
	    */
        public static WxPayData AppRefundQuery(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/pay/refundquery";
            //检测必填参数
            if (!inputObj.IsSet("out_refund_no") && !inputObj.IsSet("out_trade_no") &&
            ...
            Log.Debug("WxPayApi", "RefundQuery request : " + xml);
            string response = AppHttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口以提交数据到API
            Log.Debug("WxPayApi", "RefundQuery response : " + response);
